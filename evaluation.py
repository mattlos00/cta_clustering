...
def eva(y_true, y_pred, sil, epoch=0):
    global bestari
    global bestsil
    acc, f1, y_pred_mapped = cluster_acc2(y_true, y_pred)
    ...
    print(
        ...
        " ari {:.4f},".format(ari),
        " f1 {:.4f},".format(f1),
        " Number of Clusters {:.4f},".format(n_clus),
        " Best ari {:.4f},".format(bestari),
        " Best sil_score {:.4f}".format(bestsil),
    )
    """
    ...
    print(
        ...
        + str(statistics.median(count))
    )
    print(
        "Epoch " + str(epoch) + ": Mean cluster count = " + str(statistics.mean(count))
    )
    return ari, acc, y_pred_mapped, n_clus, bestsil
